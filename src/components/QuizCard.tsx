...
const QuizCard = ({question, answer, options, q_no}: QuizCardProps) => {
    const {nextQ, prevQ, updateAnswerList, goToQ, answerList} = useContext(QuizContext)
        ...
        <div className=" relative h-screen pb-40 flex px-5 flex-col justify-center items-center bg-gray-900 shadow-2xl text-white">
           ...
           <div className='flex justify-center w-full  pb-10'>
                {Array(10).fill('').map((_, index) => {
                    const color = answerList[index]!== "0"? "bg-green-500" : "bg-white"
                    return (
                        <button onClick={() => goToQ(index+1)} className={color+` flex mx-1 w-10 h-10 md:mx-1.5 justify-center items-center md:w-20 md:h-20 text-black rounded-lg shadow-sm shadow-slate-300 hover:shadow-2xl hover:shadow-slate-200 hover:scale-110 transition-all duration-200 active:scale-105  cursor-pointer`}>
            ...
            <h1 className=" flex text-start text-2xl md:text-4xl mt-32  pb-5 font-bold  w-full mx-3 px-2 border-solid border-2 border-green-500 p-3 mb-5 rounded-lg">Q{q_no}. {decodedQuestion}</h1>
            ...
            {options?.map((option: string) => {
                ...
                return (
                    <div className=" z-20 justify-center w-[400px] items-center mb-5">
                        <button onClick={()=>{updateAnswerList(option,q_no-1)}} className="text-white w-full mx-5 transition-all duration-200  focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-xl md:text-2xl px-5 py-2.5 mr-2 mb-2 bg-blue-600 hover:bg-green-700 focus:outline-none focus:bg-green-500  cursor-pointer">{option}</button>
                    </div>
                )
            })
            }
